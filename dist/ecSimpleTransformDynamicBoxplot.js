(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      factory((global.ecSimpleTransformDynamicBoxplot = {})));
  ...
  var ResultDimInfoInternal = (function () {
    function ResultDimInfoInternal(
      index,
      indexInUpstream,
      method,
      name,
    ...
    ResultDimInfoInternal.prototype.gatherValue = function (
      groupByDimInfo,
      groupVal,
      value
    ) {
  ...
  function prepareDimensions(config, upstream, groupByDimInfo) {
    ...
    for (var i = 0; i < resultDimensionsConfig.length; i++) {
      ...
      var dimInfoInUpstream = upstream.getDimensionInfo(
      ...
      assert(
        ...
        "Dimension ".concat(
          ...
          ' is the "groupBy" dimension, must not have any "method".'
        )
      );
      ...
      var name_1 =
        resultDimInfoConfig.name != null
          ...
          : dimInfoInUpstream.name;
      var finalResultDimInfo = new ResultDimInfoInternal(
  ...
  function travel(
    ...
    if (groupByDimInfo) {
      mapByGroup = {};
      for (
        var dataIndex = 0, len = upstream.count();
        dataIndex < len;
        dataIndex++
      ) {
        var groupByVal = upstream.retrieveValue(
          dataIndex,
      ...
      for (
        var dataIndex = 1, len = upstream.count();
        dataIndex < len;
        dataIndex++
      ) {
  ...
  function normalizeMethod(method) {
    ...
    methodInternal = hasOwn(METHOD_ALIAS, methodInternal)
      ...
      : methodInternal;
    assert(
      hasOwn(METHOD_INTERNAL, methodInternal),
      "Illegal method ".concat(method, ".")
    );
    return methodInternal;
  }
  var createCollectionResultLine = function (
    upstream,
    dataIndex,
    collectionDimInfoList,
    groupByDimInfo,
    ...
    for (var i = 0; i < collectionDimInfoList.length; i++) {
      ...
      for (var j = 0; j < collectionInfoList.length; j++) {
        ...
        newLine[collectionInfo.indexInLine] = +lineCreator[
  ...
  var updateCollectionResultLine = function (
    upstream,
    dataIndex,
    targetLine,
    collectionDimInfoList,
  ...
  var createFinalResultLine = function (
    upstream,
    dataIndex,
    finalResultDimInfoList,
    groupByDimInfo,
  ...
  var updateFinalResultLine = function (
    upstream,
    dataIndex,
    targetLine,
    finalResultDimInfoList,
...
//# sourceMappingURL=ecSimpleTransformDynamicBoxplot.js.map
